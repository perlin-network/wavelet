...
func (b *broadcaster) MakeTransaction(tag uint32, payload []byte) *wire.Transaction {
	...
	b.LedgerDo(func(l wavelet.LedgerInterface) {
		parents, err = l.FindEligibleParents()
		if err == nil {
			nonce, err = b.Wallet.NextNonce(l.(*wavelet.Ledger))
		}
	})
...
func (b *broadcaster) BroadcastTransaction(wired *wire.Transaction) {
	...
	log.Debug().Str("id", hex.EncodeToString(id)).Uint32("tag", wired.Tag).Msg("Successfully broadcasted transaction.")
}
