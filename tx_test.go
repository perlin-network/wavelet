...
func TestTransaction_ExpectedDifficulty(t *testing.T) {
	...
	assert.Equal(t, byte(9), Transaction{Depth: 54638, Confidence: 196398}.ExpectedDifficulty(MinDifficulty))

	// Genesis transaction.
	assert.Equal(t, MinDifficulty, Transaction{Depth: 0, Confidence: 0}.ExpectedDifficulty(MinDifficulty))

	// Test upper bounds.
	assert.Equal(t, byte(34), Transaction{Depth: 19638, Confidence: math.MaxUint64}.ExpectedDifficulty(MinDifficulty))
